...
<head>
    ...
    <div class="col-lg-9">
          <div class="container">
              <form class="d-fle" Action="Display-Product.php" method="post">
              ...
              <table class="table">
                ...
                <tbody>
                  ...
                  if(isset($_POST['submit'])){
                      $search=$_POST['search'];
                      $req="SELECT*FROM product WHERE nom LIKE '%$search%'";
                      $res=$conx->query($req);
                      if($row=$res->fetch_assoc()){
                          ?>
                        <tr>
                  <td><img src="<?=$row['images']?>" alt="" width="50px" height="50px"></td>
                    <td><?=$row['nom']?></td>
                    <td><?=$row['price']?> UM</td>
                    <td><?=$row['quantite']?></td>
                    <td>
                    <a class="btn btn-primary" href="edit-product.php?id=<?=$row['id']?>"><i class="bi bi-pencil-square"></i></a>
                  ...
                  $star= 0; 
                  $end=6; 
                  $r=$conx->query("SELECT*FROM product ");
                  $nb = $r->num_rows;
                  $page=ceil($nb/$end);
                  ...
                  $req="SELECT*FROM product LIMIT $star,$end";
                  $res=$conx->query($req);
